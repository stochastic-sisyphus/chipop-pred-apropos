...
class FeatureEngineering:
    ...
    def calculate_growth_rates(self, df: pd.DataFrame, value_cols: List[str], 
    ...
    def create_retail_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'median_household_income' in df.columns:
                retail_df['retail_spending_potential'] = df['median_household_income'] * 0.3  # Assumed 30% of income
            
            # Calculate retail gap
            if all(col in retail_df.columns for col in ['retail_space_per_capita', 'retail_spending_potential']):
                retail_df['retail_gap'] = (
    ...
    def create_development_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            if 'estimated_cost' in df.columns:
                dev_df['development_intensity'] = df['estimated_cost'] / df.groupby('year')['estimated_cost'].transform('mean')
    ...
    def create_economic_features(self, df: pd.DataFrame, fred_data: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """
            ...
            for series_id, fred_df in fred_data.items():
                if series_id == 'CUUR0200SA0':  # CPI
                    ...
                    econ_df['price_to_national_ratio'] = (
                        ...
                        fred_df['value'].iloc[-1]
                    )
            
            # Calculate economic health score
            if all(col in econ_df.columns for col in ['median_household_income', 'unemployment_impact']):
                econ_df['economic_health_score'] = (
                    econ_df['median_household_income'] * 
        ...
        try:
            ...
            for col in value_cols:
                if col in df.columns:
                    # 3-year momentum
                    temp_df[f'{col}_momentum_3y'] = (
                        df[col] - df[col].shift(3)
                    ) / df[col].shift(3)
                    
                    # 5-year momentum
                    temp_df[f'{col}_momentum_5y'] = (
        ...
        try:
            ...
            if all(col in df.columns for col in growth_features):
                ...
                score_df['growth_potential_score'] = np.average(
                    normalized_growth,
                    weights=settings.GROWTH_POTENTIAL_WEIGHTS,
                    axis=1
                )
            ...
            if all(col in df.columns for col in retail_features):
                ...
                score_df['retail_opportunity_score'] = np.average(
                    normalized_retail,
                    weights=settings.RETAIL_OPPORTUNITY_WEIGHTS,
                    axis=1
                )
            ...
            if all(col in df.columns for col in housing_features):
                ...
                score_df['housing_demand_score'] = np.average(
                    normalized_housing,
                    weights=settings.HOUSING_DEMAND_WEIGHTS,
                    axis=1
                )
    ...
    def engineer_all_features(self, df: pd.DataFrame, fred_data: Dict[str, pd.DataFrame],
        ...
        try:
            ...
            if 'development_density' not in feature_df.columns:
                if 'total_permits' in feature_df.columns and 'total_population' in feature_df.columns:
                    with np.errstate(divide='ignore', invalid='ignore'):
                        feature_df['development_density'] = feature_df['total_permits'] / feature_df['total_population']
                else:
