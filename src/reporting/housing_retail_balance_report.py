...
REQUIRED_COLS = [
    'total_population', 'total_housing_units', 'retail_space', 'retail_demand', 'retail_gap', 'retail_supply', 'vacancy_rate'
]
...
class HousingRetailBalanceReport:
    ...
    def generate_report(self, census_data, permit_data, economic_data, zoning_data, retail_metrics, retail_deficit):
        try:
            ...
            merged = census_data.merge(retail_metrics, on='zip_code', how='left', suffixes=('', '_retail'))
            ...
            metrics = {
                ...
                'avg_retail_density': float(merged['retail_space'].sum() / merged['total_population'].sum()) if merged['total_population'].sum() else 0,
                'avg_housing_density': float(merged['total_housing_units'].sum() / merged['total_population'].sum()) if merged['total_population'].sum() else 0,
        ...
        return template.render(
            ...
            current_analysis={
                'housing': {
                    ...
                    'density': metrics['total_housing_units']
