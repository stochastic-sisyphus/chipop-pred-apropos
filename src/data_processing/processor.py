...
class DataProcessor:
    ...
    def process_population_data(self) -> bool:
        ...
        try:
            ...
            df = (
                ...
                .agg({"population": "sum", "households": "sum", "median_income": "mean"})
                .reset_index()
            )
    ...
    def process_economic_data(self, df):
        ...
        try:
            ...
            for col in numeric_cols:
                if (
                    col in df.columns
                    and (missing_pct := (df[col].isna().sum() / len(df)) * 100) is not None
                ):
    ...
    def _merge_processed_files(self, results: Dict[str, bool]) -> Optional[pd.DataFrame]:
        ...
        for key, df in processed_files.items():
            if "zip_code" in df.columns:
                processed_files[key] = df[df["zip_code"].isin(settings.CHICAGO_ZIP_CODES)]
        ...
        try:
            ...
            if "retail_business_count" in merged.columns and "retail_space" in merged.columns:
                merged["retail_supply"] = merged["retail_business_count"] * (merged["retail_space"] / merged["retail_business_count"]).fillna(0)
            ...
            if "retail_gap" in merged.columns and "retail_demand" in merged.columns:
                merged["retail_lag"] = merged["retail_gap"] / merged["retail_demand"].replace(0, np.nan)
            # Add robust error handling for missing/insufficient data
            for col in ["retail_permits", "retail_construction_cost", "retail_business_count", "retail_space", "retail_demand", "retail_supply", "retail_gap", "retail_lag"]:
                if col not in merged.columns:
                    merged[col] = np.nan
                if merged[col].isnull().all():
                    logging.warning(f"All values missing for {col} in merged retail data.")
            return merged
        except Exception as e:
            logging.error(f"Failed to merge processed retail files: {e}")
            return None

    def _load_processed_files(self, results: Dict[str, bool]) -> Dict[str, pd.DataFrame]:
    ...
    def _initialize_base_dataset(self, processed_files: Dict[str, pd.DataFrame]) -> Optional[pd.DataFrame]:
    ...
    def _merge_permits_data(self, merged_df: pd.DataFrame, processed_files: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        """Merge permits data and compute retail metrics."""
        if "permits" not in processed_files:
            return merged_df

        try:
            permits_df = self._prepare_permits_data(processed_files["permits"])
            permits_df = self._compute_retail_metrics(permits_df, merged_df)
            merged_df = pd.merge(merged_df, permits_df, on=["zip_code", "year"], how="left")
            merged_df = self._fill_missing_permit_values(merged_df)
        except Exception as e:
            logger.error(f"Error merging permits: {str(e)}")
            merged_df = self._add_missing_permit_columns(merged_df)
    ...
    def _handle_missing_retail_data(self, permits_df: pd.DataFrame) -> None:
        ...
        if (permits_df['retail_permits'] > 0).sum() == 0:
            logger.warning("No retail_permits > 0 found; substituting retail_permits from commercial_permits * 0.2")
            permits_df['retail_permits'] = permits_df['commercial_permits'] * 0.2

        if (permits_df['retail_construction_cost'] > 0).sum() == 0:
            logger.warning("No retail_construction_cost > 0 found; estimating from commercial_construction_cost * 0.2")
            permits_df['retail_construction_cost'] = permits_df['commercial_construction_cost'] * 0.2

    def _compute_retail_metrics(self, permits_df: pd.DataFrame, merged_df: pd.DataFrame) -> pd.DataFrame:
        """Compute retail-related metrics."""
        retail_space = permits_df['retail_construction_cost'] / 200
        retail_demand = merged_df['total_population'] * merged_df['median_household_income'] * 0.3
    ...
    def _merge_business_licenses(self, merged_df: pd.DataFrame, processed_files: Dict[str, pd.DataFrame]) -> pd.DataFrame:
        ...
        try:
            ...
            merged_df = pd.merge(merged_df, licenses_df, on=["zip_code", "year"], how="left")
        except Exception as e:
            logger.error(f"Error merging business_licenses: {str(e)}")
            
        return merged_df

    def _merge_economic_data(self, merged_df: pd.DataFrame, processed_files: Dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    def _verify_and_initialize_columns(self, merged_df: pd.DataFrame) -> None:
        ...
        for col, default_val in retail_columns.items():
            if col not in merged_df.columns:
                ...
                logger.warning(f"{col} not found - initialized with default value {default_val}")
    ...
    def process_all(self) -> bool:
        ...
        try:
            ...
            # Process economic data
            economic_success = self.process_economic_data(pd.read_csv(settings.RAW_DATA_DIR / "economic_indicators.csv"))
            ...
            try:
                for col in ["total_population", "total_housing_units"]:
                    if col in df.columns:
                        ...
                        logger.info(f"{col}: {stagnant} ZIPs have no change over period")
            except Exception as e:
    ...
    def process_business_licenses(self) -> bool:
        ...
        try:
            ...
            processed_df = processed_df.rename(
                columns={"license_id": "total_licenses", "account_number": "unique_businesses"}
            )
    ...
    def generate_retail_metrics(self) -> pd.DataFrame:
        ...
        try:
            ...
            retail_business_count_path = settings.RAW_DATA_DIR / "retail_business_count.csv"
            ...
            if retail_sqft_path.exists():
                ...
                retail_sqft_per_zip_df['zip_code'] = retail_sqft_per_zip_df['zip_code'].astype(str).str.zfill(5)
            else:
                logger.warning("retail_sqft_per_zip.csv not found; retail_space will fallback to zero.")
            if retail_business_count_path.exists():
                retail_business_count_df = pd.read_csv(retail_business_count_path)
                retail_business_count_df['zip_code'] = retail_business_count_df['zip_code'].astype(str).str.zfill(5)
            else:
                logger.warning("retail_business_count.csv not found; retail_supply will fallback to zero.")
            ...
            if (df["retail_space"] == 0).all():
                logger.warning("All values in retail_space are zero after merge. Attempting enrichment using fallback formulas.")
            ...
            if (df["retail_supply"] == 0).all():
                logger.warning("All values in retail_supply are zero after merge. Attempting enrichment using fallback formulas.")
    ...
    def process_zoning_data(self, df) -> bool:
        ...
        try:
            ...
            metrics = (
                df.groupby("zip_code")
                .agg({"total_parcels": "sum", "avg_lot_size": "mean", "total_area": "sum"})
                .reset_index()
            )
    ...
    def process_multifamily_permits(self) -> pd.DataFrame:
        ...
        mf_agg = df.groupby(["zip_code", "year"]).agg({"unit_count": "sum"}).reset_index()
        mf_agg.to_csv(settings.PROCESSED_DATA_DIR / "multifamily_permits_processed.csv", index=False)
        logger.info(f"Saved processed multifamily permits to {settings.PROCESSED_DATA_DIR / 'multifamily_permits_processed.csv'}")
        return mf_agg
    ...
    def process_retail_opportunity_areas(self):
        ...
        mf = pd.read_csv(settings.PROCESSED_DATA_DIR / "multifamily_permits_processed.csv")
        ...
        hist = mf[mf["year"] <= mid_year].groupby("zip_code")["unit_count"].sum().reset_index(name="hist_units")
        recent = mf[mf["year"] > mid_year].groupby("zip_code")["unit_count"].sum().reset_index(name="recent_units")
        growth = pd.merge(hist, recent, on="zip_code", how="outer").fillna(0)
        growth["growth_pct"] = np.where(growth["hist_units"] > 0, (growth["recent_units"] - growth["hist_units"]) / growth["hist_units"], np.where(growth["recent_units"] > 0, 1.0, 0))
        # Merge with permits for retail lag
        permits["zip_code"] = permits["zip_code"].astype(str).str.zfill(5)
        permits["retail_lag"] = permits["retail_permits"] / (permits["residential_permits"] + 1e-6)
        # Merge with growth
        ...
        top5.to_csv(settings.PROCESSED_DATA_DIR / "retail_opportunity_areas.csv", index=False)
        logger.info(f"Saved top 5 retail opportunity ZIPs to {settings.PROCESSED_DATA_DIR / 'retail_opportunity_areas.csv'}")
        return top5
    ...
    def run_pipeline(self) -> bool:
        ...
        try:
            ...
            results = {
                ...
                "permits": self.process_permits_data(),
                "business_licenses": self.process_business_licenses(),
                "economic": self.process_economic_data(pd.read_csv(settings.RAW_DATA_DIR / "economic_indicators.csv")),
            }
    ...
    def process_retail_data(self, data: pd.DataFrame) -> bool:
        ...
        try:
            ...
            retail_metrics["retail_demand"] = (
                retail_metrics["total_population"] * retail_metrics["median_household_income"] * 0.3
            )
    ...
    def extract_zip(self, address, community_area=None, ward=None):
        ...
        if community_area is not None:
            logger.warning(f"No ZIP found, using community_area {community_area} as proxy for ZIP")
    ...
    def validate_zip_codes(self, df, zip_col="zip_code"):
        ...
        if percent_valid < 80:
            logger.warning(f"Low valid ZIP percentage ({percent_valid:.2f}%). Attempting ZIP recovery.")
            def recover_zip(row):
                if re.match(r"^606\d{2}$", str(row[zip_col])):
                    return row[zip_col]
                # Try extracting from address fields
                address_fields = [row.get("street_number", ""), row.get("street_direction", ""), row.get("street_name", ""), row.get("contact_1_city", "Chicago")] 
            ...
            valid = df[zip_col].astype(str).str.match(r"^606[0-9]{2}$")
            percent_valid = valid.mean() * 100
            logger.info(f"Post-recovery valid Chicago ZIPs: {percent_valid:.2f}% ({valid.sum()} of {len(df)})")
    ...
    def enrich_with_retail_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        required = ["retail_space", "retail_demand", "retail_gap", "retail_supply", "vacancy_rate"]
        warnings = []

        enrichment_rules = {
            "retail_space": {
                "retail_construction_cost": lambda x: x["retail_construction_cost"] / 200,
                "retail_permits": lambda x: x["retail_permits"] * 10000
            },
            "retail_demand": {
                "total_population,median_household_income": lambda x: x["total_population"] * x["median_household_income"] * 0.3
            },
            "retail_gap": {
                "retail_demand,retail_supply": lambda x: x["retail_demand"] - x["retail_supply"]
            },
            "retail_supply": {
                "retail_space": lambda x: x["retail_space"] * 300
            },
            "vacancy_rate": {
                "default": lambda _: 0.1
            }
        }
    ...
    def save_outputs(self, merged_df: pd.DataFrame):
        ...
        # Write outputs
        merged_df.to_csv(settings.PROCESSED_DATA_DIR / "merged_dataset.csv", index=False)
