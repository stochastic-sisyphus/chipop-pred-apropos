    ...
    def analyze_zoning_patterns(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            development_potential = (
                df.groupby(zip_col)
                .agg({"zoning_area": "sum", "building_area": "sum"})
                .reset_index()
            )

            development_potential["development_capacity"] = (
                development_potential["zoning_area"] - development_potential["building_area"]
            )

            return {"zoning_mix": zoning_mix, "development_potential": development_potential}
    ...
    def analyze_property_values(self, df: pd.DataFrame) -> Dict[str, pd.DataFrame]:
        ...
        try:
            ...
            value_metrics.columns = [
                f"{col[0]}_{col[1]}" if col[1] else col[0] for col in value_metrics.columns
            ]
            ...
            value_density = (
                df.groupby(zip_col)
                .agg({"market_value": "sum", "property_area": "sum"})
                .reset_index()
            )

            value_density["value_density"] = (
                value_density["market_value"] / value_density["property_area"]
            )

            return {"value_metrics": value_metrics, "value_density": value_density}
