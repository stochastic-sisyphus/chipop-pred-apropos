...
class EconomicModel:
    ...
    def load_data(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            required_cols = ['total_population', 'median_household_income', 'total_permits', 'total_construction_cost']
            resolved_cols = [resolve_column_name(df, col, column_aliases) for col in required_cols]

            if missing_cols := [col for col, resolved in zip(required_cols, resolved_cols) if not resolved]:
            ...
            if feature_list is not None:
                ...
                for feature in optional_features:
                    if feature in df.columns:
                        ...
                        logger.warning(f"Optional feature '{feature}' not found - model will proceed without it")
