...
class Pipeline:
    ...
    def run_with_data(self, data):
        ...
        This helper bypasses the data collection step and is mainly
        used for testing or when data has already been prepared
        externally.

        Args:
            data (dict | pd.DataFrame): Pre-collected data. If a single
                ``pd.DataFrame`` is provided it will be treated as the
                combined dataset.
        ...
        try:
            if isinstance(data, pd.DataFrame):
                logger.info("Wrapping provided DataFrame into dictionary")
                data = {"census": data}

            if not data:
                logger.error("No data supplied to run_with_data")
                return self._handle_data_quality_error(
                    DataQualityError("No data supplied")
                )

            logger.info("Processing provided data…")
