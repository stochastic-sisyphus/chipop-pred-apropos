...
class Visualizer:
    ...
    def __init__(self):
        ...
        self.colors = sns.color_palette(
            "husl", n_colors=59
        )  # Match exact number of Chicago ZIP codes
        ...
        try:
            ...
            sns.barplot(
                data=data.sort_values(
                    "retail_gap" if "retail_gap" in data.columns else "zip_code", ascending=False
                ),
                x="zip_code",
    ...
    def create_development_charts(self, data: pd.DataFrame) -> None:
        ...
        try:
            ...
            if all(
                ...
                plt.bar(
                    ["Residential", "Commercial", "Retail"], permit_counts, color=self.colors[:3]
                )
                self._extracted_from__extracted_from_create_balance_analysis_charts_19_19(
            ...
            plt.figure(figsize=(12, 8))
            sns.boxplot(data=data, x="year", y="total_construction_cost", palette="viridis")
            self._extracted_from__extracted_from_create_balance_analysis_charts_19_19(
    ...
    def create_population_charts(self, data: pd.DataFrame) -> None:
        ...
        try:
            ...
            plt.figure(figsize=(12, 8))
            sns.boxplot(data=data, x="year", y="total_population", palette="viridis")
            self._extracted_from__extracted_from_create_balance_analysis_charts_19_19(
    ...
    def create_permit_analysis_charts(self) -> bool:
        ...
        try:
            ...
            res_permits_col = resolve_column_name(
                self.permit_data, "residential_permits", column_aliases
            )
            comm_permits_col = resolve_column_name(
                self.permit_data, "commercial_permits", column_aliases
            )
            retail_permits_col = resolve_column_name(
                self.permit_data, "retail_permits", column_aliases
            )
            ...
            yearly_permits = (
                self.permit_data.groupby(year_col)
                .agg({res_permits_col: "sum", comm_permits_col: "sum", retail_permits_col: "sum"})
                .reset_index()
            )
            ...
            zip_permits = self.permit_data.groupby(zip_col).agg({permits_col: "sum"}).reset_index()
    ...
    def create_economic_indicators_chart(self, output_path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            if "employment" in self.economic_data.columns:
                ...
                logger.warning(
                    f"Employment column not found in economic data. Columns: {self.economic_data.columns.tolist()}"
                )
            # Update layout
            fig.update_layout(
        ...
        try:
            ...
            if "year" not in self.scenario_data.columns:
                if self.scenario_data.index.name != "year":
                    logger.warning(
                        "'year' column not found in scenario data; using index as fallback."
                    )
            ...
            if not y_col:
                ...
                numeric_cols = [
                    c
                    for c in self.scenario_data.columns
                    if c not in ["year", "scenario"] and self.scenario_data[c].dtype.kind in "fi"
                ]
    ...
    def create_business_activity_chart(self, output_path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            y_col = (
                "active_licenses"
                if "active_licenses" in self.business_data.columns
                else ("total_licenses" if "total_licenses" in self.business_data.columns else None)
            )
            if y_col:
                fig.add_trace(
                    go.Scatter(
    ...
    def create_trend_plot(self, data, x_label, y_label, title, output_path):
        ...
        try:
            ...
            plt.tight_layout()
            return self._extracted_from_create_map_plot_21(output_path, "Created trend plot: ")
        except Exception as e:
    ...
    def _add_north_arrow(self, ax):
        ...
        try:
            ...
            # Add 'N' label
            ax.text(arrow_x, arrow_y + arrow_length * 0.2, "N", ha="center", va="bottom")
    ...
    def _extracted_from_create_balance_analysis_charts_19(self, arg0, arg1, arg2):
        self._extracted_from__extracted_from_create_balance_analysis_charts_19_19(arg0, arg1, arg2)
    ...
    def _extracted_from__extracted_from_create_balance_analysis_charts_19_19(
        self, arg0, arg1, arg2
    ):
        plt.title(arg0)
        plt.xlabel(arg1)
